...
namespace WOM_EYE.Providers.Notes
{
	public class NoteProvider : INoteProvider
	{
		...
		{
			string spName = "spWOMEYE_GetAllCatatan";
        ...
        public List<NoteModel> getAllCatatan(string uid, string userId)
        {
			string spName = "spWOMEYE_GetAllCatatanNew";

			var dataCatatan = _dbConnection.ExecuteReader(spName, new { @uid = uid, @userId = userId}, commandType: CommandType.StoredProcedure, commandTimeout: 30);
			...
			while (dataCatatan.Read())
			{
				...
				catatan.USR_UPD = dataCatatan[6].ToString();
				catatan.DTM_UPD = dataCatatan[7].ToString();
				catatan.IS_ACCESS  = dataCatatan[8].ToString();
				catatan.IS_EDIT  = dataCatatan[9].ToString();
