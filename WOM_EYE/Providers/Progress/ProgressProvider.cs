...
namespace WOM_EYE.Providers
{
	public class ProgressProvider : IProgressProvider
	{
        ...
        public List<ProgressModel> getAllProgress(string uid, string userId)
        {
			string spName = "spWOMEYE_GetAllProgressNew";

			var dataProgress = _dbConnection.ExecuteReader(spName, new { @uid = uid, @userId = userId }, commandType: CommandType.StoredProcedure, commandTimeout: 30);
			...
			while (dataProgress.Read())
			{
				...
				Progress.USR_UPD = dataProgress[6].ToString();
				Progress.DTM_UPD = dataProgress[7].ToString();
				Progress.IS_ACCESS = dataProgress[8].ToString();
				Progress.IS_EDIT = dataProgress[8].ToString();
				Progress.IS_DELETE = dataProgress[10].ToString();
				_listProgress.Add(Progress);
			}
        ...
        public ProgressModel getDataProgressById(string id, string userId)
        {
			string sp = "spWOMEYE_GetProgressByIdNew";

			var resp = _dbConnection.QueryFirstOrDefault<ProgressModel>(sp, new { @id = id, @userId = userId }, commandType: CommandType.StoredProcedure, commandTimeout: 30);
